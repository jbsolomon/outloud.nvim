...
3. Transcribes audio locally via `llama-server` + Voxtral Mini 3B GGUF
...
| `src/transcribe/mod.rs` | `SpeechTranscriber` trait definition. |
| `src/transcribe/http.rs` | HTTP backend: WAV encoding, `/v1/audio/transcriptions` endpoint, `/v1/chat/completions` fallback. |
...
1. **Local-only STT** — No cloud dependency. Voxtral Mini 3B runs on-device via `llama-server`.
...
4. **Auto-managed llama-server** — The plugin spawns, probes, and tears down `llama-server`. Model auto-downloads from HuggingFace on first run.
5. **Full teardown on `VimLeavePre`** — Quitting Neovim always kills the daemon and llama-server.
